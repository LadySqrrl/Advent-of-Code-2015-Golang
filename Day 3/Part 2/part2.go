...
func main() {
	...
	tempSanta, tempRobo := "", ""
	santaX, santaY, roboX, roboY, count := 0, 0, 0, 0, 0

	var coordsVisited []string
	coordsVisited = append(coordsVisited, "0, 0")
	...
	for i := 0; i < len(path); i++ {
		...
		if count%2 == 0 {
			...
			tempSanta = strconv.Itoa(santaX) + ", " + strconv.Itoa(santaY)
			coordsVisited = append(coordsVisited, tempSanta)
			...
			tempRobo = strconv.Itoa(roboX) + ", " + strconv.Itoa(roboY)
			coordsVisited = append(coordsVisited, tempRobo)
			fmt.Println(tempRobo)
		}
	...
	minusDuplicates := removeDuplicatesUnordered(coordsVisited)

	atLeastOnce := len(minusDuplicates)

	fmt.Println(atLeastOnce)
}
